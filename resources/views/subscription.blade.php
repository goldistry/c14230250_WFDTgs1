...
@section('content')
    <div class="min-h-screen bg-gradient-to-b from-black via-neutral-950 to-black text-gray-300 font-sans antialiased">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24">
            <h1
                class="text-4xl sm:text-5xl lg:text-6xl font-semibold text-center text-white mb-16 sm:mb-20 tracking-tight animate-fade-in-up-scale delay-[100ms]">
                Choose Your Plan
            </h1>
                ...
                @foreach ($subscriptions as $subscription)
                    <div x-data="{ showDetails: false }"
                        ...
                        <div class="relative z-10 flex flex-col flex-grow">
                            <div class="flex-grow">
                                <div class="flex items-center space-x-4 mb-4">
                                    @if ($subscription['logo'])
                                        <img src="{{ asset('storage/' . $subscription['logo']) }}"
                                            alt="{{ $subscription['name'] }} Logo"
                                            class="w-11 h-11 object-contain rounded-lg bg-neutral-800 p-1 shadow-md">
                                    @else
                                        <div
                                            class="w-11 h-11 rounded-lg bg-neutral-800 flex items-center justify-center shadow-md">
                            ...
                            <div x-show="showDetails" x-collapse.duration.500ms
                                class="pt-6 mt-6 border-t border-neutral-800/60 group-hover:border-orange-600/30 transition-colors duration-300"
                                @click.stop>
                                <h3 class="text-xs uppercase tracking-wider font-semibold text-neutral-500 mb-4">Pricing
                                </h3>
                                <div class="space-y-4 mb-6">
                                    <div
                                        class="grid grid-cols-{{ $subscription['monthly_price'] && $subscription['yearly_price'] ? '2' : '1' }} gap-4">
                                        @if ($subscription['monthly_price'])
                                            <div
                                                class="flex flex-col items-center justify-center p-4 bg-neutral-800 rounded-lg">
                                                <span class="text-sm text-neutral-400">Monthly</span>
                                                <span
                                                    class="mt-2 text-2xl font-bold text-white">{{ $subscription['monthly_price'] }}</span>
                                            </div>
                                        @endif
                                        @if ($subscription['yearly_price'])
                                            <div
                                                class="flex flex-col items-center justify-center p-4 bg-gradient-to-tr from-orange-600/10 via-neutral-800 to-neutral-800 border border-orange-600/30 rounded-lg relative overflow-hidden">
                                                <span
                                                    class="absolute top-1 right-1 text-[10px] bg-orange-500 text-white px-1.5 py-0.5 rounded-full shadow-sm">SAVE</span>
                                                <span class="text-sm text-neutral-400">Yearly</span>
                                                <span
                                                    class="mt-2 text-2xl font-bold text-white">{{ $subscription['yearly_price'] }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                <h3 class="text-xs uppercase tracking-wider font-semibold text-neutral-500 mb-3 mt-6">
                                    Features</h3>
                                <ul class="space-y-2 text-sm text-neutral-400">
                                    <li class="flex items-center">
                                        <svg class="w-4 h-4 mr-2 text-orange-500/80 flex-shrink-0" fill="currentColor"
                                            viewBox="0 0 20 20">
                                    ...
                                    <li class="flex items-center text-neutral-500">
                                        <svg class="w-4 h-4 mr-2 text-neutral-600 flex-shrink-0" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
